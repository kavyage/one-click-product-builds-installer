...
import static InstallerProperties.P_P5

public class P5Product extends BaseProduct {
    public P5Product() {
        super(P_P5);
    }
    ...
    public void stop() {
        ...
        for (String pid : CmdUtils.getPID("java", "P5")) killProcess(pid)
    }

    @Override
    void install() {
        def path = InstallerProperties.getLocalBuildsPath()
        def filesToCleanUp = CmdUtils.getListFilesInFolderByRegExTemplate(path, "P5Installer.exe|xec.*|setx.*")
